...
feature 'Admin ends a rental ' do
  ...
  scenario 'I calculate total value' do
    user = User.create!(email: 'teste@teste.com.br', password:'12345678')
    ...
    expect(page).to have_content("Valor de cada diária R$ 100,00")
    expect(page).to have_content("Valor por dia do seguro R$ 25,00")
    expect(page).to have_content("Valor por dia do seguro para terceiros R$ 13,00 ")
    expect(page).to have_content('Valor Total R$ 276,00')
  end
  scenario 'should ensure that the minimum amount is always charged' do
    user = User.create!(email: 'teste@teste.com.br', password:'12345678')
    ...
    travel_to Time.zone.local(2020, 05, 01, 12, 30, 00) do
      click_on 'Encerrar Locação'
    end
    ...
    expect(page).to have_content('Valor Total R$ 138,00')
  end
end  